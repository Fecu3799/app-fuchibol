...
export class MatchLifecycleJob {
  ...
  async runTick(): Promise<void> {
    this.metrics?.incCounter('cron_jobs_runs_total');
    if (this.redis) {
      void this.redis
        .set('lifecycle:lastTickAt', new Date().toISOString(), 'EX', 120)
        .catch(() => undefined);
    }
  ...
  private async autoGenerateTeams(match: MatchWithParticipants): Promise<void> {
    try {
      ...
      this.metrics?.incCounter('cron_jobs_failures_total');
      this.logger.warn(
  ...
  private async autoLock(match: MatchWithParticipants): Promise<void> {
    try {
      ...
      if (newRevision !== null) {
        this.realtimePublisher.notifyMatchUpdated(match.id, newRevision);
        this.logger.log({
          op: 'lifecycleTransition',
          matchId: match.id,
          fromState: match.status,
          toState: 'locked',
          trigger: 'cron',
        });
        this.metrics?.incCounter('match_lifecycle_transitions_total', {
          toState: 'locked',
        });
      }
    } catch (err: unknown) {
  ...
  private async autoStart(
    ...
    try {
      ...
      if (newRevision !== null) {
        const toState = isLate ? 'played' : 'in_progress';
        this.realtimePublisher.notifyMatchUpdated(match.id, newRevision);
        this.logger.log({
          op: 'lifecycleTransition',
          matchId: match.id,
