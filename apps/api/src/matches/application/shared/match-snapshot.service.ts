...
async function buildMatchSnapshot(
...
export class MatchSnapshotService {
  ...
  /** For queries: uses this.prisma.client + URL building with StorageService */
  async build(matchId: string, actorId: string): Promise<MatchSnapshot> {
    return buildMatchSnapshot(this.prisma.client, matchId, actorId, (key) =>
      this.storage.buildPublicUrl(key),
    );
  }

  /** For mutations inside transactions: receives tx, no URL building (URLs = null) */
  async buildInTx(
    tx: TransactionClient,
    matchId: string,
    actorId: string,
  ): Promise<MatchSnapshot> {
    return buildMatchSnapshot(tx, matchId, actorId);
  }
}
